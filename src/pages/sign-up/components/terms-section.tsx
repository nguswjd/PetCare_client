...
function TermsSection({ terms }: TermsSectionProps) {
  return (
    <div className="w-full flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-3">
        <Checkbox
          label="모든 약관에 동의합니다."
          checked={terms.checkedAll}
          onCheckedChange={(v: boolean) => terms.toggleAll(v)}
        />
        ...
        <div className="flex flex-col gap-4">
          <Checkbox
            ...
            expandable
            variant="primary"
            checked={terms.checkedTerms.service}
            ...
            expanded={terms.expandedKey === "service"}
            onCheckedChange={(v) => terms.toggleOne("service", v)}
            onExpandChange={() => terms.toggleExpand("service")}
          />
          <Checkbox
            label="(필수) 개인정보 수집 및 이용 동의"
            expandable
            variant="primary"
            checked={terms.checkedTerms.privacy}
            ...
            expanded={terms.expandedKey === "privacy"}
            onCheckedChange={(v) => terms.toggleOne("privacy", v)}
            onExpandChange={() => terms.toggleExpand("privacy")}
          />
          <Checkbox
            label="(필수) 위치 서비스 활용 동의"
            variant="primary"
            expandable={false}
            checked={terms.checkedTerms.location}
            onCheckedChange={(v) => terms.toggleOne("location", v)}
          />
          <Checkbox
            label="(선택) 마케팅 활용 및 광고성 정보 수신 동의"
            expandable={false}
            variant="primary"
            checked={terms.checkedTerms.marketing}
            onCheckedChange={(v) => terms.toggleOne("marketing", v)}
          />
        </div>
      </div>
    </div>
